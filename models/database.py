...
# Define caminho do banco (usa .env ou padrão)
...
def init_db(db_name: str = DB_PATH) -> None:
    with connect(db_name) as conn:
        ...
        # Verifica se a coluna "concluida" existe
        cursor = conn.cursor()
        ...
        if "concluida" not in colunas:
            cursor.execute(
                "ALTER TABLE tarefas ADD COLUMN concluida INTEGER DEFAULT 0;"
            )
            conn.commit()
...
class Database:
    """
    Classe que gerencia conexões e operações com SQLite.
    Permite uso com: with Database() as db:
    """
